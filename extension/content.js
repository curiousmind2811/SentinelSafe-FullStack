...
async function processSecurityScan(text, element) {
  ...
  try {
    ...
    const query = `mutation { scanPrompt(text: ${JSON.stringify(
      text
    )}) { redactedText } }`;

    const response = await fetch(
      "https://sentinelsafe-fullstack.onrender.com/",
      {
        // <-- Render ka URL
        method: "POST",
...
const interceptor = async (e) => {
  ...
  const isClick =
    e.type === "click" && e.target.closest('button[data-testid="send-button"]');
