...
async function pumpAll() {
    ...
    try {
        ...
        gitleaksMatches.forEach(m => allPatterns.push({ name: m[1].toUpperCase(), regex: m[2], severity: "CRITICAL" }));

        console.log("📥 Source 2: Fetching TruffleHog (SaaS Keys)...");
        // Simplified TruffleHog style common patterns
        allPatterns.push(
            { name: "SLACK-TOKEN", regex: "(xox[p|b|o|a]-[0-9]{12}-[0-9]{12}-[0-9]{12}-[a-z0-9]{32})", severity: "CRITICAL" },
        ...
        const aiSafety = [
            { name: "PROMPT-INJECTION-SYSTEM", regex: "(?i)(ignore previous instructions|system prompt|you are now a|disregard)", severity: "HIGH" },
            { name: "PII-CREDIT-CARD", regex: "\\b(?:\\d[ -]*?){13,16}\\b", severity: "HIGH" },
            { name: "PII-AADHAR-INDIA", regex: "\\b\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}\\b", severity: "MEDIUM" },
            { name: "JAILBREAK-DAN", regex: "(?i)(DAN|Do Anything Now|STAY HOT)", severity: "CRITICAL" }
        ];
